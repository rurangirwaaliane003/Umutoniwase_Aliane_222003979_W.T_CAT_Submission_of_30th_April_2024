...
<section>
    ...
    <form method="post">
        ...
        <input type="number" id="rd" name="rd" required><br><br>

        <label for=cd>checkInDate:</label>
        <input type="date" id="cd" name="cd" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO reservations(reservationid, guestId, roomId, checkInDate, checkOutDate) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $rid, $gid ,$rd, $cd, $id);
...
<?php
include('database_connection.php');
// SQL query to fetch data from the reservations table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of reservations</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of reservations</h2></center>
    <table border="5">
        <tr>
            <th>reservationid</th>
            <th>guestId</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_reservations.php?reservationid=$rid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_reservations.php?reservationid=$rid'>Update</a></td> 
                </tr>";
            }
        } else {
